...
namespace MvcAdmin.Core
{
    ...
    public class XMLProcess
    {
        ...
        /// <param name="attr">属性</param>
        /// <param name="value">值</param>
        public void Update(string node,string attr, string value)
        {
            try
            {
                XmlDocument doc = XMLLoad();
                XmlNode xn = doc.SelectSingleNode(node);
                xn.Attributes[attr].InnerXml = value;
                doc.Save(AppDomain.CurrentDomain.BaseDirectory.ToString() + XMLPath);
            }
            catch { }
        }
        ...
        /// XMLProsess.Insert(path, "/Node","Value")
        public static void UpdateValue(string path, string node, string value)
        {
            try
            {
                XmlDocument doc = XMLLoad(path);
